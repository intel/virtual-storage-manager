...
def sum_performance_metrics(context, search_opts, session=None):#for iops bandwidth
    ...
    while timestamp_cur<timestamp_end:
        sql_str = '''
            ...
            FROM
            (select m.value-m_pre.value as value_real ,m.instance as instance_real
              from (select timestamp,instance,hostname,value from metrics WHERE metrics.metric = '%(metrics_name)s' AND metrics.timestamp >= %(time_1)s AND metrics.timestamp < %(time_2)s) as m
              left join (select timestamp,instance,hostname,value from metrics WHERE metrics.metric = '%(metrics_name)s' AND metrics.timestamp >= %(time_1)s-20 AND metrics.timestamp < %(time_2)s-20) as m_pre
        ...
        for cell in sql_ret_set:
            ...
            if metrics_name in ['osd_op_in_bytes','osd_op_out_bytes']:
                metrics_value = metrics_value and metrics_value*0.1/1024/1024 or 0
...
def lantency_performance_metrics(context, search_opts, session=None):#for lantency
    ...
    session = get_session()
    while timestamp_cur < timestamp_end:
        sql_str = '''select sum(latencytable.lantency_value),count(latencytable.lantency_value)  FROM (
