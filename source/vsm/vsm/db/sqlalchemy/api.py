    ...
    while timestamp_cur<timestamp_end:
        sql_str = '''select  sum(iops_value) as sum_iops,sum(lantency_value* iops_value) as total_lantency,iopstable.instance  from \
            (\
              ...
              inner join (select case when avgcount<>0 then value/avgcount else 0 end as lantency_value,a.hostname,a.instance,a.timestamp from \
				(select * from metrics where metric ='%s_sum') as a \
				left join \
				(select timestamp,value as avgcount from metrics where metric='%s_avgcount') as b \
            ...
            '''%(lantency_type,metrics_name,metrics_name,metrics_name,timestamp_cur,timestamp_cur+15)
        timestamp_cur = timestamp_cur + 15
