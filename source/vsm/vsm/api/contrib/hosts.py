...
class HostController(object):
    ...
    def __init__(self):
        self.api = storage_api.HostAPI()
        super(HostController, self).__init__()

    @wsgi.serializers(xml=HostIndexTemplate)
    def index(self, req):
        authorize(req.environ['vsm.context'])
    ...
    def _set_enabled_status(self, req, host, enabled):
        """Sets the specified host's ability to accept new storages."""
        context = req.environ['vsm.context']
    ...
    def show(self, req, id):
        """Shows the storage usage info given by hosts.
        ...
        :returns: expected to use HostShowTemplate.
            ex.::
                ...
                D: {'host': 'hostname','project': 'admin',
                    'storage_count': 1, 'total_storage_gb': 2048}
        """
        host = id
        context = req.environ['vsm.context']
        ...
        storage_refs = db.storage_get_all_by_host(context, host_ref['host'])
        (count, sum) = db.storage_data_get_for_host(context,
                                                   host_ref['host'])

        snap_count_total = 0
        snap_sum_total = 0
        resources = [{'resource': {'host': host, 'project': '(total)',
                      'storage_count': str(count),
                      'total_storage_gb': str(sum),
                      'snapshot_count': str(snap_count_total),
                      'total_snapshot_gb': str(snap_sum_total)}}]

        project_ids = [v['project_id'] for v in storage_refs]
        project_ids = list(set(project_ids))
        for project_id in project_ids:
            (count, sum) = db.storage_data_get_for_project(context, project_id)
...
class Hosts(extensions.ExtensionDescriptor):
    ...
    name = "Hosts"
    alias = "os-hosts"
    namespace = "http://docs.openstack.org/storage/ext/hosts/api/v1.1"
