...
from vsmclient import shell
from vsmclient.v1 import shell as shell_v1
...
class ShellTest(utils.TestCase):
    ...
    FAKE_ENV = {
        'VSM_USERNAME': 'username',
        'VSM_PASSWORD': 'password',
        'VSM_PROJECT_ID': 'project_id',
        'OS_VOLUME_API_VERSION': '1.1',
        'VSM_URL': 'http://no.where',
    }
    ...
    def test_list(self):
        ...
        self.assert_called('GET', '/vsms/detail')

    def test_list_filter_status(self):
        self.run_command('list --status=available')
        self.assert_called('GET', '/vsms/detail?status=available')

    def test_list_filter_display_name(self):
        self.run_command('list --display-name=1234')
        self.assert_called('GET', '/vsms/detail?display_name=1234')

    def test_list_all_tenants(self):
        self.run_command('list --all-tenants=1')
        self.assert_called('GET', '/vsms/detail?all_tenants=1')

    def test_show(self):
        self.run_command('show 1234')
        self.assert_called('GET', '/vsms/1234')

    def test_delete(self):
        self.run_command('delete 1234')
        self.assert_called('DELETE', '/vsms/1234')

    def test_snapshot_list_filter_vsm_id(self):
        self.run_command('snapshot-list --vsm-id=1234')
        self.assert_called('GET', '/snapshots/detail?vsm_id=1234')

    def test_snapshot_list_filter_status_and_vsm_id(self):
        self.run_command('snapshot-list --status=available --vsm-id=1234')
        self.assert_called('GET', '/snapshots/detail?'
                           'status=available&vsm_id=1234')

    def test_rename(self):
        # basic rename with positional agruments
        self.run_command('rename 1234 new-name')
        expected = {'vsm': {'display_name': 'new-name'}}
        self.assert_called('PUT', '/vsms/1234', body=expected)
        # change description only
        self.run_command('rename 1234 --display-description=new-description')
        expected = {'vsm': {'display_description': 'new-description'}}
        self.assert_called('PUT', '/vsms/1234', body=expected)
    ...
    def test_set_metadata_set(self):
        ...
        self.assert_called('POST', '/vsms/1234/metadata',
                           {'metadata': {'key1': 'val1', 'key2': 'val2'}})

    def test_set_metadata_delete_dict(self):
        self.run_command('metadata 1234 unset key1=val1 key2=val2')
        self.assert_called('DELETE', '/vsms/1234/metadata/key1')
        self.assert_called('DELETE', '/vsms/1234/metadata/key2', pos=-2)
