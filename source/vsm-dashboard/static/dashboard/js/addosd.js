...
function DeleteOSD(osd_id){
	if(confirm("Are you sure that you want to delete it?")){
		PostData("delete_osd_action",{"osd_id":osd_id});
	}
...
function UpdateOSDList(osd_list){
	...
	for(var i=0;i<osd_list.length;i++){
		...
		tbodyHtml += "	<td class='sortable normal_column'>";
		tbodyHtml += "		<button class=\"btn btn-danger\" onclick=\"DeleteOSD("+osd_list[i].id+")\">Delete</button>";
		tbodyHtml += "</td>";
		tbodyHtml += "</tr>";
