...
vsm.datatables = {
  update: function () {
    var $rows_to_update = $('tr.ajax-update');
    if ($rows_to_update.length) {
      ...
      if ($rows_to_update.find('.actions_column .btn-group.open').length) {
        ...
        setTimeout(vsm.datatables.update, interval);
      ...
      $rows_to_update.each(function(index, row) {
        ...
        horizon.ajax.queue({
          ...
          error: function (jqXHR, textStatus, errorThrown) {
            switch (jqXHR.status) {
              ...
              case 404:
                ...
                row_count = vsm.datatables.update_footer_count($table, -1);
          ...
          complete: function (jqXHR, textStatus) {
            ...
            vsm.datatables.validate_button();
            ...
            next_poll = interval * decay_constant * 2;
            // Limit the interval to 30 secs
            if(next_poll > 30 * 1000) { next_poll = 30 * 1000; }
            //setTimeout(vsm.datatables.update, 5000);
          }
        });
      });
      setTimeout(vsm.datatables.update, 4500);
    }
  },
...
vsm.datatables.confirm = function (action) {
  var $action = $(action),
...
vsm.datatables.disable_buttons = function() {
...
vsm.datatables.update_footer_count = function (el, modifier) {
  var $el = $(el),
...
vsm.datatables.add_no_results_row = function (table) {
...
vsm.datatables.remove_no_results_row = function (table) {
 ...
 **/
vsm.datatables.fix_row_striping = function (table) {
  table.trigger('applyWidgetId', ['zebra']);
};

vsm.datatables.set_table_sorting = function (parent) {
    // ubind horizon sorting <
    $(".header").unbind("click");
...
vsm.datatables.add_table_checkboxes = function(parent) {
...
vsm.datatables.set_table_query_filter = function (parent) {
    ...
    if (input.length > 0) {
      ...
      input.quicksearch(table_selector + ' tbody tr', {
        ...
        onBefore: function () {
          ...
          vsm.datatables.remove_no_results_row(table);
        },
        onAfter: function () {
          var template, table, colspan, params;
          table = $(table_selector);
          vsm.datatables.update_footer_count(table);
          vsm.datatables.add_no_results_row(table);
          vsm.datatables.fix_row_striping(table);
        },
...
vsm.datatables.set_table_fixed_filter = function (parent) {
...
horizon.addInitFunction(function() {
  vsm.datatables.validate_button();
  vsm.datatables.disable_buttons();
  $('table.datatable').each(function (idx, el) {
    vsm.datatables.update_footer_count($(el), 0);
  });
...
//  vsm.datatables.add_table_checkboxes($('body'));
  vsm.datatables.set_table_sorting($('body'));
//  vsm.datatables.set_table_query_filter($('body'));
//  vsm.datatables.set_table_fixed_filter($('body'));
//
//  // Also apply on tables in modal views.
//  horizon.modals.addModalInitFunction(vsm.datatables.add_table_checkboxes);
//  horizon.modals.addModalInitFunction(vsm.datatables.set_table_sorting);
//  horizon.modals.addModalInitFunction(vsm.datatables.set_table_query_filter);
//  horizon.modals.addModalInitFunction(vsm.datatables.set_table_fixed_filter);
//
//  // Also apply on tables in tabs views for lazy-loaded data.
//  //horizon.tabs.addTabLoadFunction(vsm.datatables.add_table_checkboxes);
//  //horizon.tabs.addTabLoadFunction(vsm.datatables.set_table_sorting);
//  //horizon.tabs.addTabLoadFunction(vsm.datatables.set_table_query_filter);
//  //horizon.tabs.addTabLoadFunction(vsm.datatables.set_table_fixed_filter);

  vsm.datatables.update();
});
